...
SSL 인증서를 SSL 인증서 화면에 가져오고 나면 언제든 해당 인증서를 보고 업데이트할 수 있습니다. 업데이트한 모든 세부사항은 간격과 행 바꾸기를 포함하여 원본 인증서와 정확히 일치해야 하므로 업데이트는 [가져오기 프로세스](/docs/infrastructure/ssl-certificates?topic=ssl-certificates-importing-ssl-certificates#importing-ssl-certificates)와 비슷하게 작동합니다.
{:shortdesc}
...
3. **조치** 드롭 다운 목록에서 **인증서 보기/업데이트**를 선택하십시오.
4. 해당 텍스트 상자에서 **SSL 인증서 세부사항**을 업데이트하고 **업데이트**를 클릭하여 변경사항을 적용하십시오. 자세한 정보는 다음 표를 참조하십시오. 
